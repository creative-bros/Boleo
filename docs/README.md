# Manuales Boleo

Este directorio reúne los manuales operativos base para compartir acceso al portal Boleo con usuarios de prueba, administradores y equipo de revisión.

## Contenido

- [01-acceso-y-primer-ingreso.md](./01-acceso-y-primer-ingreso.md)
- [02-gestion-de-usuarios-y-unidades.md](./02-gestion-de-usuarios-y-unidades.md)
- [03-cobranza-y-reportes.md](./03-cobranza-y-reportes.md)
- [04-amenidades-y-mantenimiento.md](./04-amenidades-y-mantenimiento.md)
- [05-pruebas-y-retroalimentacion.md](./05-pruebas-y-retroalimentacion.md)
- [06-solicitud-de-conversion-a-docx.md](./06-solicitud-de-conversion-a-docx.md)
- [08-api-solicitudes-cotizacion.md](./08-api-solicitudes-cotizacion.md)

## Recomendación de envío

Para compartir la plataforma con un tercero, se recomienda enviar:

1. URL del sistema.
2. Usuario y contraseña temporal.
3. Los manuales `.md` de este directorio.
4. El formato de retroalimentación del archivo `05-pruebas-y-retroalimentacion.md`.
5. Si probaran el formulario web, el contrato del archivo `08-api-solicitudes-cotizacion.md`.

## Orden sugerido de lectura

1. Acceso y primer ingreso.
2. Gestión de usuarios y unidades.
3. Cobranza y reportes.
4. Amenidades y mantenimiento.
5. Pruebas y comentarios.
